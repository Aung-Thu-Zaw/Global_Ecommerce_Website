...
const handleRestoreTrashFaqSubCategory = async (trashFaqSubCategoryId) => {
  const result = await swal({
    icon: "question",
    title: "Are you sure you want to restore this faq subcategory?",
  ...
  if (result.isConfirmed) {
    router.post(
      route("admin.faq-categories.sub-categories.restore", {
...
const handleDeleteTrashFaqSubCategory = async (trashFaqSubCategoryId) => {
  const result = await swal({
    ...
    text: "Faq SubCategory in the trash will be permanetly deleted! You can't get it back.",
    showCancelButton: true,
  ...
  if (result.isConfirmed) {
    router.delete(
      route("admin.faq-categories.sub-categories.force.delete", {
        trash_faq_sub_category_id: trashFaqSubCategoryId,
        page: params.page,
        per_page: params.per_page,
        sort: params.sort,
...
const handlePermanentlyDeleteTrashFaqCategoires = async () => {
  const result = await swal({
    icon: "question",
    title: "Are you sure you want to delete it from the trash?",
    text: "All faq subcategories in the trash will be permanetly deleted! You can't get it back.",
    showCancelButton: true,
    confirmButtonText: "Yes, Delete it !",
    confirmButtonColor: "#d52222",
    cancelButtonColor: "#626262",
  ...
  if (result.isConfirmed) {
    router.get(
      route("admin.faq-categories.sub-categories.permanently.delete", {
        page: params.page,
        per_page: params.per_page,
        sort: params.sort,
        direction: params.direction,
...
<template>
  <AdminDashboardLayout>
    <Head title="Trash Faq SubCategories" />
        ...
        <tbody v-if="trashFaqSubCategories.data.length">
          ...
          >
            ...
            <Td v-if="faqCategoryTrashRestore || faqCategoryTrashDelete">
              ...
              <button
                v-if="faqCategoryTrashDelete"
                @click="handleDeleteTrashFaqSubCategory(trashFaqSubCategory.id)"
