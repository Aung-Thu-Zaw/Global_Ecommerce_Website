    ...
    <div class="px-4 md:px-10 mx-auto w-full py-32">
      ...
      <div class="flex items-center justify-end mb-5">
        ...
        <DashboardSearchInputForm
          href="admin.orders.shipped.index"
          placeholder="SEARCH_BY_INVOICE"
        />

        <!-- Perpage Select Box -->
        <div class="ml-5">
          <DashboardPerPageSelectBox href="admin.orders.shipped.index" />
        </div>

        <!-- Filter By Date -->
        <DashboardFilterByCreatedDate href="admin.orders.shipped.index" />
      </div>
      ...
      <TableContainer>
        ...
        <tbody v-if="shippedOrders.data.length">
          <Tr v-for="shippedOrder in shippedOrders.data" :key="shippedOrder.id">
            ...
            <Td>
              {{ shippedOrder.invoice_no }}
            </Td>

            <Td class="capitalize">
              {{ shippedOrder.payment_type }}
            </Td>

            <Td> $ {{ formattedAmount(shippedOrder.total_amount) }} </Td>
            ...
            <Td v-if="orderManageDetail">
              ...
              <div>
                <DetailButton
                  href="admin.orders.shipped.show"
                  :id="shippedOrder.id"
                />
              </div>
            </Td>
      ...
      <!-- Shipped Order Table End -->

      <!-- No Data Row -->
      <NotAvaliableData v-if="!shippedOrders.data.length" />

      <!-- Pagination -->
      <div v-if="shippedOrders.data.length" class="mt-6">
        <p class="text-center text-sm text-gray-600 mb-3 font-bold">
          Showing {{ shippedOrders.from }} - {{ shippedOrders.to }} of
          {{ shippedOrders.total }}
        </p>
        <Pagination :links="shippedOrders.links" />
      </div>
    </div>
  </AdminDashboardLayout>
</template>
