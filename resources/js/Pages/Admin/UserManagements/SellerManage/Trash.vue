...
const handleRestoreTrashSeller = async (sellerId) => {
  const result = await swal({
    icon: "question",
    title: __("ARE_YOU_SURE_YOU_WANT_TO_RESTORE_THIS_SELLER"),
...
const handleDeleteTrashSeller = async (sellerId) => {
  const result = await swal({
    ...
    text: __(
      "SELLER_IN_THE_TRASH_WILL_BE_PERMANETLY_DELETED_YOU_CANT_GET_IT_BACK"
    ),
    showCancelButton: true,
...
const handlePermanentlyDeleteTrashSellers = async () => {
  const result = await swal({
    ...
    title: __("ARE_YOU_SURE_YOU_WANT_TO_DELETE_IT_FROM_THE_TRASH"),
    text: __(
      "ALL_SELLERS_IN_THE_TRASH_WILL_BE_PERMANETLY_DELETED_YOU_CANT_GET_IT_BACK"
    ),
    showCancelButton: true,
    confirmButtonText: __("YES_DELETE_IT"),
    cancelButtonText: __("CANCEL"),
    ...
    <div class="px-4 md:px-10 mx-auto w-full py-32">
      ...
      <div class="flex items-center justify-end mb-5">
        ...
        <DashboardSearchInputForm
          href="admin.seller-manage.trash"
          placeholder="SEARCH_BY_SHOP_NAME"
        />
        <!-- Perpage Select Box -->
        <div class="ml-5">
          <DashboardPerPageSelectBox href="admin.seller-manage.trash" />
        </div>

        <!-- Filter By Date -->
        <DashboardFilterByDeletedDate href="admin.seller-manage.trash" />
      </div>

      <p
        v-if="sellerManageTrashDelete && trashSellers.data.length !== 0"
        class="text-left text-sm font-bold mb-2 text-warning-600"
      >
        {{
          __("SELLERS_IN_THE_TRASH_WILL_BE_AUTOMATICALLY_DELETED_AFTER_60_DAYS")
        }}

        <EmptyTrashButton @click="handlePermanentlyDeleteTrashSellers" />
      </p>
      ...
      <TableContainer>
        ...
        <tbody v-if="trashSellers.data.length">
          <Tr v-for="trashSeller in trashSellers.data" :key="trashSeller.id">
            ...
            <Td
              v-if="sellerManageTrashRestore || sellerManageTrashDelete"
              class="flex items-center"
            >
              <!-- Restore Button -->
              <div v-if="sellerManageTrashRestore">
                <RestoreButton
              ...
              <!-- Delete Button -->
              <div v-if="sellerManageTrashDelete">
                <DeleteForeverButton
