...
const params = reactive({
  search: null,
  page: props.townships.current_page ? props.townships.current_page : 1,
  per_page: props.townships.per_page ? props.townships.per_page : 10,
  sort: "id",
  direction: "desc",
});
...
const handleDelete = async (township) => {
  const result = await swal({
    icon: "warning",
    title: "Are you sure you want to delete this township?",
    text: "You will be able to restore this township in the trash!",
    ...
    <div class="px-4 md:px-10 mx-auto w-full py-32">
      <div class="flex items-center justify-between mb-10">
        ...
        <div>
          <Link
            as="button"
            :href="route('admin.townships.trash')"
      ...
      <div class="mb-5 flex items-center justify-between">
        <Link
          :href="route('admin.townships.create')"
          :data="{
            per_page: params.per_page,
          }"
      ...
      <TableContainer>
        <TableHeader>
          ...
          <HeaderTh @click="updateSorting('name')"> Country </HeaderTh>
          <HeaderTh @click="updateSorting('name')"> Region </HeaderTh>
          <HeaderTh @click="updateSorting('name')"> City </HeaderTh>
        ...
        <tbody v-if="townships.data.length">
          <Tr v-for="township in townships.data" :key="township.id">
            <BodyTh>{{ township.id }}</BodyTh>
            <Td>{{ township.city.region.country.name }}</Td>
            <Td>{{ township.city.region.name }}</Td>
            ...
            <Td>
              <Link
                as="button"
                :href="route('admin.townships.edit', township.slug)"
                :data="{
                  page: props.townships.current_page,
                  per_page: params.per_page,
                }"
