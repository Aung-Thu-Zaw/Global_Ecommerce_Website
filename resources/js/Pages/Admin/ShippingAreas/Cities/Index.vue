...
const handleDeleteCity = async (city) => {
  if (city.townships.length > 0) {
    const result = await swal({
      icon: "error",
      title: __("YOU_CANT_DELETE_THIS_CITY_BECAUSE_THIS_CITY_HAVE_TOWNSHIPS"),
      text: __(
        "IF_YOU_CLICK_THE_DELETE_WHATEVER_BUTTON_TOWNSHIPS_ASSOCIATED_WITH_THAT_CITY_WILL_BE_AUTOMATICALLY_DELETED"
      ),
    ...
    const result = await swal({
      icon: "question",
      title: __("ARE_YOU_SURE_YOU_WANT_TO_DELETE_THIS_CITY"),
      text: __("YOU_WILL_BE_ABLE_TO_RESTORE_THIS_CITY_IN_THE_TRASH"),
      showCancelButton: true,
    ...
    <div class="px-4 md:px-10 mx-auto w-full py-32">
      <div class="flex items-center justify-between mb-10">
        ...
        <div v-if="shippingAreaTrashList">
          <TrashButton href="admin.cities.trash" />
        </div>
      </div>
      ...
      <div class="mb-5 flex items-center justify-between">
        ...
        <div v-if="shippingAreaAdd">
          <CreateButton href="admin.cities.create" name="ADD_CITY" />
        </div>
        ...
        <div class="flex items-center ml-auto">
          ...
          <!-- Perpage Select Box -->
          <div class="ml-5">
            <DashboardPerPageSelectBox href="admin.cities.index" />
          </div>

          <!-- Filter By Date -->
          <DashboardFilterByCreatedDate href="admin.cities.index" />
        </div>
      </div>
          ...
          <Tr v-for="city in cities.data" :key="city.id">
            ...
            >
              ...
              <div v-if="shippingAreaEdit">
                <EditButton href="admin.cities.edit" :slug="city.slug" />
              </div>

              <!-- Delete Button -->
              <div v-if="shippingAreaDelete">
                <DeleteButton @click="handleDeleteCity(city)" />
              </div>
            </Td>
          </Tr>
        </tbody>
      ...
      <!-- No Data Row -->
      <NotAvaliableData v-if="!cities.data.length" />

      <!-- Pagination -->
      <div v-if="cities.data.length" class="mt-6">
        <p class="text-center text-sm text-gray-500 mb-3 font-bold">
          Showing {{ cities.from }} - {{ cities.to }} of
          {{ cities.total }}
        </p>
