...
const params = reactive({
  search: null,
  page: props.regions.current_page ? props.regions.current_page : 1,
  per_page: props.regions.per_page ? props.regions.per_page : 10,
  sort: "id",
  direction: "desc",
});
...
const handleDelete = async (region) => {
  if (region.cities.length > 0) {
    const result = await swal({
      icon: "error",
      title: "You can't delete this region because this region have cities?",
      text: "If you click 'Delete, whatever!' button cities will be automatically deleted.You will be able to restore this region in the trash!",
    ...
    if (result.isConfirmed) {
      router.delete(
        route("admin.regions.destroy", {
          region: region.slug,
          page: props.regions.current_page,
          per_page: params.per_page,
        })
      );
      setTimeout(() => {
    ...
    const result = await swal({
      ...
      text: "You will be able to restore this region in the trash!",
      showCancelButton: true,
    ...
    <div class="px-4 md:px-10 mx-auto w-full py-32">
      <div class="flex items-center justify-between mb-10">
        ...
        <div>
          <Link
            as="button"
            :href="route('admin.regions.trash')"
        ...
        <tbody v-if="regions.data.length">
          <Tr v-for="region in regions.data" :key="region.id">
            ...
            <Td>
              <Link
                as="button"
                :href="route('admin.regions.edit', region.slug)"
                :data="{
                  page: props.regions.current_page,
                  per_page: params.per_page,
                }"
