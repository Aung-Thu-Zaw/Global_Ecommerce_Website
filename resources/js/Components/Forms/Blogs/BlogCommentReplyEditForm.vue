...
const form = useForm({
  ...
  reply_text: props.blogComment.blog_comment_reply?.reply_text,
  captcha_token: null,
});

const { executeRecaptcha, recaptchaLoaded } = useReCaptcha();
const handleEditBlogCommentReply = async () => {
  await recaptchaLoaded();
  form.captcha_token = await executeRecaptcha("edit_blog_comment_reply");
  form.patch(
    route(
      "blogs.comments.reply.update",
      props.blogComment.blog_comment_reply?.id
    ),
    {
      replace: true,
      preserveScroll: true,
...
<template>
  <div class="w-full">
    <form @submit.prevent="handleEditBlogCommentReply">
      <div>
        <textarea
          cols="30"
          rows="10"
