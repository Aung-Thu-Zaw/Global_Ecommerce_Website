...
<template>
  <TableContainer class="w-full mt-5">
    ...
    <tbody v-if="approvedCancelOrders.length">
      <Tr v-for="order in approvedCancelOrders" :key="order.id">
  ...
  <div v-if="!approvedCancelOrders.length" class="p-5 w-full bg-gray-100">
    <p class="text-center text-sm uppercase text-slate-500 font-bold">
      There is no approved return orders.
    </p>
  </div>
</template>
